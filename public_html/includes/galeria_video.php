		...
		<div id="box">
			<div id="videos">
					<?php if(!isset($_GET['video'])){
						...
						echo '<iframe title="'.$qr['titulo'].'" width="437px" height="338px" src="http://www.youtube.com/embed/'.$qr['embed'].'"frameborder="0" allowfullscreen></iframe>';
					   }
					   elseif(isset($_GET['video'])){
						   $get = $_GET['video'];
						   $selVideo = mysql_query("SELECT * FROM eneej_videos WHERE id = '$get'");
						   $qrdois = mysql_fetch_array($selVideo);
						   echo '<iframe title="'.$qrdois['titulo'].'" width="450px" height="338px" src="http://www.youtube.com/embed/'.$qrdois['embed'].'"frameborder="0" allowfullscreen></iframe>';
						}
					?>
